...
Turn the red switch to the **right position** and then press the **red button** on the left side of the red switch to power the USB Hub Controller Chip otherwise the keyboard **can't be used!** At the same time the indicator for the battery voltage will be turned on.
Also, NOTE that turning the power off does not fully turn off the system, you may notice that the battery indicator is still ON which means the power controller is delivering power which may cause your display buffer to get stuck and not usdate image or seem like the display broke by displaying lines. To resolve simply double-press the battery indicator button (left side of the power switch) to turn the power off, alternatively, the battery can be removed to clear the display controller. 
...
Put the red switch to the lower position, connect the keyboard to your computer, and open [VIAL page](https://vial.rocks/), then you can customize the keymap yourself.  
...
You can install Kali, Raspberrypi OS, Retropi, DietPi, and many others on the HackberryPi.  
